...
def welcome_email(to: str, fname: str, temp_password: str) -> None:
    html = f"""
    <p>Hi {fname},</p>
    <p>Your C&S Tools account has been created. Use the credentials below to log in and set a new password.</p>
    <p><b>Email:</b> {to}<br><b>Temporary password:</b> {temp_password}</p>
    <p><a href="{settings.FRONTEND_URL}">Log in to C&S Tools</a></p>
    <p>You will be prompted to change your password on first login.</p>
    """
    send_email_task.delay(to, "Welcome to C&S Tools", html)


def reset_password_email(to: str, fname: str, temp_password: str) -> None:
    html = f"""
    <p>Hi {fname},</p>
    <p>An administrator has reset your C&S Tools password. Use the credentials below to log in and set a new password.</p>
    <p><b>Email:</b> {to}<br><b>Temporary password:</b> {temp_password}</p>
    <p><a href="{settings.FRONTEND_URL}">Log in to C&S Tools</a></p>
    <p>You will be prompted to change your password on first login.</p>
    """
    send_email_task.delay(to, "Your C&S Tools password has been reset", html)


def password_changed_email(to: str, fname: str) -> None:
    html = f"""
    <p>Hi {fname},</p>
    <p>Your C&S Tools password was successfully changed.</p>
    <p>If you did not make this change, contact your administrator immediately.</p>
    """
    send_email_task.delay(to, "Your C&S Tools password has been changed", html)
...
def budget_complete_email(to: str, fname: str, association_name: str, budget_year: int) -> None:
    ...
    <p><a href="{settings.FRONTEND_URL}">Log in to C&S Tools to download</a></p>
    """
